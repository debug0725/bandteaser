...
function renderRules(){
  const cinemaRule = typeof CINEMA_RECITAL !== "undefined" ? CINEMA_RECITAL : "이것은 참다운 법으로써 여래를 공양하는 길이오니 나라를 다 바치고 처자로 보시하여도 이것이 제일의 보시이옵니다";
  const inCinema = document.body.classList.contains("cinema-mode");
  $("rulesContent").innerHTML = RULES.map((rule,i)=>{
    const unlocked = inCinema ? true : i < state.rulesRead;
    const falseMark = !inCinema && state.falseRuleFound && i === 4;
    const cinemaShown = i < 10 ? cinemaRule : i === 10 ? "해당 영화관은 현재 재난관리국에서 발견되지 않은 신규 괴담으로 추정됩니다" : "당장 자살하십시오";
    const shown = inCinema ? cinemaShown : (unlocked ? corruptText(rule) : "████████████████");
    const cinemaClass = inCinema ? (i === 11 ? "cinema-overwritten-rule cinema-suicide-rule" : "cinema-overwritten-rule") : "";
    return `<div class="rule-item ${unlocked?"":"locked"} ${falseMark?"false-rule":""} ${cinemaClass}">
      <strong>${String(i+1).padStart(2,"0")}</strong><br>
      ${shown}
    </div>`;
  }).join("") + `<div class="rule-item">${inCinema ? "※ 1번부터 10번까지 같은 문장으로 덮어쓰기 되었습니다." : "※ 이 수칙 중 일부는 거짓입니다."}</div>`;
}
...
function renderScene(id){
  ...
  choices.forEach((choice,index)=>{
    ...
    btn.addEventListener("click",()=>{
      ...
      if(choice.key === "cinema_tear"){
        playCinemaTicketTear(choice.next);
        updateStatus();
        return;
      }
...
function corruptAgentName(name, pollution){
  const chars=[...name];
  let blocks=0;
  if(pollution>=95) blocks=Math.max(3,Math.ceil(chars.length*.75));
  else if(pollution>=85) blocks=Math.max(2,Math.ceil(chars.length*.5));
  else if(pollution>=70) blocks=1;
  if(!blocks) return name;
  const order=[...chars.keys()].sort((a,b)=>((a*7+3)%11)-((b*7+3)%11));
  for(let i=0;i<Math.min(blocks,chars.length);i++) chars[order[i]]="█";
  return chars.join("");
}

function updateAgentBadge(){
  const name = state.keyword || "미등록";
  const displayName = corruptAgentName(name,state.pollution);
  const top = $("agentCardNameTop");
  const field = $("agentCardName");
  const ending = $("agentCardEnding");
  const pollution = $("agentCardPollution");
  const badge = $("agentBadge");
  const endingData = state.endingId ? ENDINGS[state.endingId] : null;
  if(top) top.textContent = displayName;
  if(field) field.textContent = displayName;
  if(ending) ending.textContent = endingData ? `${endingData.code} · ${endingData.title}` : "미기록";
  if(pollution) pollution.textContent = `${state.pollution}%`;
  if(badge){
    badge.dataset.pollution = String(state.pollution);
    badge.classList.toggle("corrupt-low",state.pollution>=50);
    badge.classList.toggle("corrupt-mid",state.pollution>=70);
    badge.classList.toggle("corrupt-high",state.pollution>=85);
    badge.classList.toggle("corrupt-max",state.pollution>=95);
  }
}

let agentPhotoDataUrl = "";

function loadAgentPhoto(file){
  ...
  reader.onload = () => {
    agentPhotoDataUrl = String(reader.result || "");
    const preview = $("agentPhotoPreview");
    preview.src = agentPhotoDataUrl;
    preview.classList.add("loaded");
    $("agentPhotoPlaceholder").classList.add("hidden");
    toast("요원 사진을 적용했습니다.");
  };
  reader.onerror = () => toast("사진을 불러오지 못했습니다.");
...
function drawCanvasPortraitCorruption(ctx,x,y,w,h,pollution){
  if(pollution<50) return;
  ctx.save();
  const density=pollution>=95?420:pollution>=85?230:pollution>=70?120:55;
  ctx.globalAlpha=pollution>=95?.38:pollution>=85?.24:.14;
  for(let i=0;i<density;i++){
    const nx=x+seededNoise(i+pollution)*w;
    const ny=y+seededNoise(i*2+pollution+17)*h;
    const nw=1+Math.floor(seededNoise(i*3+9)*8);
    const shade=seededNoise(i*5+3)>.5?"#000":"#fff";
    ctx.fillStyle=shade;ctx.fillRect(nx,ny,nw,1+Math.floor(seededNoise(i*7+2)*3));
  }
  if(pollution>=85){
    ctx.globalAlpha=pollution>=95?.9:.7;
    ctx.fillStyle="#050505";
    ctx.filter="blur(4px)";
    ctx.beginPath();ctx.ellipse(x+w*.37,y+h*.38,w*.095,h*.035,0,0,Math.PI*2);ctx.fill();
    ctx.beginPath();ctx.ellipse(x+w*.63,y+h*.38,w*.095,h*.035,0,0,Math.PI*2);ctx.fill();
    ctx.filter="none";
  }
  ctx.restore();
}
...
async function saveResult(){
  updateAgentBadge();
  try{ await document.fonts.load('700 32px "Gowun Batang"'); }catch(e){}
  const canvas=document.createElement("canvas");
  canvas.width=1260; canvas.height=600;
  const ctx=canvas.getContext("2d");
  if(!ctx){ toast("이미지를 생성할 수 없습니다."); return; }

  ctx.fillStyle="#f8fafc"; roundedRect(ctx,8,8,1244,584,30); ctx.fill();
  ctx.strokeStyle="#cbd5e1";ctx.lineWidth=4;roundedRect(ctx,8,8,1244,584,30);ctx.stroke();

  ctx.save();ctx.translate(960,230);ctx.rotate(-15*Math.PI/180);ctx.textAlign="center";
  ctx.fillStyle="rgba(225,29,72,.055)";ctx.font='900 150px "Gowun Batang", serif';ctx.fillText("RESTRICTED",0,0);ctx.restore();

  ctx.fillStyle="#e2e8f0";roundedRect(ctx,540,24,180,14,7);ctx.fill();ctx.strokeStyle="#94a3b8";ctx.lineWidth=2;ctx.stroke();
  ctx.fillStyle="#ef4444";ctx.beginPath();ctx.arc(1198,58,9,0,Math.PI*2);ctx.fill();

  const px=70,py=105,pw=290,ph=365;
  ctx.fillStyle="#e2e8f0";ctx.fillRect(px,py,pw,ph);
  if(agentPhotoDataUrl){
    const img=new Image(); img.src=agentPhotoDataUrl;
    try{ await img.decode(); ctx.save(); ctx.filter=`grayscale(1) contrast(${state.pollution>=70?1.7:1.3}) blur(${state.pollution>=85?3:state.pollution>=70?1.2:0}px)`; drawCoverImage(ctx,img,px,py,pw,ph); ctx.restore();
      drawCanvasPortraitCorruption(ctx,px,py,pw,ph,state.pollution); }
    catch(e){ ctx.fillStyle="#64748b";ctx.font='700 30px "Gowun Batang", serif';ctx.textAlign="center";ctx.fillText("PHOTO ERROR",px+pw/2,py+ph/2); }
  }else{
    ctx.fillStyle="#64748b";ctx.font='700 30px "Gowun Batang", serif';ctx.textAlign="center";ctx.fillText("PHOTO",px+pw/2,py+ph/2-12);ctx.fillText("REQUIRED",px+pw/2,py+ph/2+28);
  }
  ctx.strokeStyle="#0f172a";ctx.lineWidth=6;ctx.strokeRect(px,py,pw,ph);
  ctx.fillStyle="#0f172a";ctx.fillRect(px,495,pw,16);

  const left=410;
  ctx.textAlign="left";ctx.fillStyle="#64748b";ctx.font='900 20px "Gowun Batang", serif';ctx.fillText("[ 재난관리국 출동구조반 ]",left,118);
  ctx.strokeStyle="#cbd5e1";ctx.lineWidth=2;ctx.beginPath();ctx.moveTo(left,142);ctx.lineTo(1160,142);ctx.stroke();
  ctx.fillStyle="#0f172a";ctx.font='700 56px "Gowun Batang", serif';ctx.fillText(corruptAgentName(state.keyword || "미등록 요원",state.pollution),left,205);
  ctx.fillStyle="#e11d48";ctx.font='800 22px "Gowun Batang", serif';ctx.fillText("[FIELD AGENT]",left,248);

  ctx.font='700 20px "Gowun Batang", serif';ctx.fillStyle="#64748b";
  ctx.fillText("이름:",left,305);ctx.fillText("혈액형:",790,305);ctx.fillText("소속반:",left,350);ctx.fillText("직급:",790,350);
  ctx.fillStyle="#0f172a";ctx.fillText(corruptAgentName(state.keyword || "미등록",state.pollution),490,305);ctx.fillText("[RH+]",900,305);ctx.fillText("[청룡]",490,350);ctx.fillText("[주무관]",900,350);

  ctx.strokeStyle="#ef4444";ctx.lineWidth=3;ctx.beginPath();ctx.moveTo(left,380);ctx.lineTo(left,450);ctx.stroke();
  const endingData = state.endingId ? ENDINGS[state.endingId] : null;
  const endingRecord = endingData ? `${endingData.code} · ${endingData.title}` : "미기록";
  ctx.fillStyle="#e11d48";ctx.font='700 17px "Gowun Batang", serif';ctx.fillText("[ 임무 기록 ]",left+18,398);
  ctx.fillStyle="#475569";ctx.font='700 18px "Gowun Batang", serif';ctx.fillText(`획득 엔딩: ${endingRecord}`,left+18,424);
  ctx.fillText(`최종 오염도: ${state.pollution}%`,left+18,448);

  ctx.fillStyle="#0f172a";ctx.fillRect(left,470,260,42);ctx.fillStyle="#f8fafc";ctx.font='800 19px "Gowun Batang", serif';ctx.fillText("재난관리국 청룡 팀",left+18,498);
  ctx.textAlign="right";ctx.fillStyle="#64748b";ctx.font='700 18px "Gowun Batang", serif';ctx.fillText("ID:[10월에 당신을 기다리겠습니다]",1160,498);

  ctx.globalAlpha=.85;ctx.fillStyle="#0f172a";
  let x=left;const widths=[3,7,2,5,3,9,2,4,7,3,2,8,4,6,2,9,3,5,2,7,4,3,8,2,6,3,9,2,5,4,7,2,8,3,5,2,9,4,6,3,8,2,5,3,7,4,9,2,6,3,8,2,4,7,3,9];
  for(let i=0;i<widths.length;i++){
    const w=widths[i];
    if(state.pollution>=70 && i%7===0){x+=12;continue;}
    const yOff=state.pollution>=85?Math.round((seededNoise(i+91)-.5)*18):0;
    const hOff=state.pollution>=95?Math.round(seededNoise(i+17)*20):0;
    ctx.fillRect(x,535+yOff,w,Math.max(4,30-hOff));
    x+=w+5;if(x>1160)break;
  }ctx.globalAlpha=1;

  const link=document.createElement("a");
  link.download=`재난관리국_사원증_${state.keyword || "미등록"}.png`;
  link.href=canvas.toDataURL("image/png");link.click();
  toast("사원증 이미지를 저장했습니다.");
}
...
$("rulesButton").addEventListener("click",()=>{
  ...
  if(document.body.classList.contains("cinema-mode")){
    dialog.classList.add("cinema-rules-reveal");
    dialog.scrollTop=0;
    const close=dialog.querySelector(".dialog-close");
    if(close) close.disabled=true;
    setTimeout(()=>{
      dialog.scrollTo({top:dialog.scrollHeight,behavior:"smooth"});
      const last=dialog.querySelector(".cinema-suicide-rule");
      last?.classList.add("cinema-rule-focus");
    },650);
    setTimeout(()=>{if(close) close.disabled=false;},3650);
  }
});
$("resetButton").addEventListener("click",()=>location.reload());
$("restartButton").addEventListener("click",()=>location.reload());
$("saveResultButton").addEventListener("click",saveResult);
$("agentPhotoInput").addEventListener("change",e=>loadAgentPhoto(e.target.files?.[0]));
...
/* ===== EXCLUSIVE SENIOR AGENT iMESSAGE ROUTES ===== */
let clueNoticeShown = false;
let clueThreadOpened = false;
let selectedSenior = null;
let clueAnswerDelivered = false;
let recoveryQueued = false;
let seniorOneCollapsed = false;
let messageTimers = [];
...
function later(fn,ms){ const id=setTimeout(fn,ms); messageTimers.push(id); return id; }
function addMessageBubble(text,type="in",sender=""){
  ...
  bubble.className=`bubble ${sender.includes("1")?"sender-one":sender.includes("2")?"sender-two":""}`;
  bubble.textContent=text;
  row.appendChild(bubble);
  thread.appendChild(row);
  thread.scrollTop=thread.scrollHeight;
}
function showMessageNotice(preview="문자 두 통이 도착했습니다."){
  const notice=$("messageNotice");
  if(!notice) return;
  $("messageNoticePreview").textContent=preview;
  notice.classList.add("show");
}
function hideMessageNotice(){ $("messageNotice")?.classList.remove("show"); }
function setMessageHeader(name,initial){
  const header=document.querySelector(".imessage-header");
  if(!header) return;
  const orb=header.querySelector(".contact-orb");
  const title=header.querySelector("strong");
  const sub=header.querySelector("small");
  if(orb) orb.textContent=initial;
  if(title) title.textContent=name;
  if(sub) sub.textContent="재난관리국 내부망";
}
function renderContactChooser(){
  setMessageHeader("메시지","메");
  const thread=$("messageThread");
  thread.innerHTML=`
    <div class="message-time">지금 · 발신자 식별 불완전</div>
    <button class="contact-choice" type="button" data-senior="1">
      <span class="contact-choice-orb one">1</span>
      <span><strong>선배 1</strong><small>윤리 규범에 다섯 번째 항목은 없어…</small></span>
      <b>›</b>
    </button>
    <button class="contact-choice" type="button" data-senior="2">
      <span class="contact-choice-orb two">2</span>
      <span><strong>선배 2</strong><small>당신은 현재 오염에 과다하게 노출됐습니다…</small></span>
      <b>›</b>
    </button>
    <p class="contact-warning">한 대화를 열면 다른 발신자의 기록은 소실됩니다.</p>`;
  $("messageReply").disabled=true;
  $("messageSend").disabled=true;
  thread.querySelectorAll("[data-senior]").forEach(btn=>btn.addEventListener("click",()=>selectSenior(btn.dataset.senior)));
}
function selectSenior(which){
  if(selectedSenior) return;
  selectedSenior=String(which);
  clueThreadOpened=true;
  clearMessageTimers();
  const thread=$("messageThread");
  thread.innerHTML='<div class="message-time">오늘 · 보안 연결 불안정</div>';
  $("messageReply").disabled=true;
  $("messageSend").disabled=true;
  if(selectedSenior==="1"){
    setMessageHeader("선배 1","1");
    later(()=>addMessageBubble("윤리 규범에 다섯 번째 항목은 없어. 나를 믿고 없다고 적어.","in","선배 1"),300);
    later(()=>{
      $("messageReply").disabled=false;
      $("messageSend").disabled=false;
      $("messageReply").placeholder="답장";
      $("messageReply").focus();
    },900);
  }else{
    setMessageHeader("선배 2","2");
    later(()=>addMessageBubble("당신은 현재 오염에 과다하게 노출됐습니다.\n그럼에도 불구하고 아직 희생자를 구하고 싶습니까?","in","선배 2"),300);
    later(()=>{
      $("messageReply").disabled=false;
      $("messageSend").disabled=false;
      $("messageReply").placeholder="‘네’라고 답장";
      $("messageReply").focus();
    },900);
  }
}
function openMessageThread(){
  ...
  if(!selectedSenior && !clueThreadOpened) renderContactChooser();
}
function triggerSeniorOneCollapse(){
  if(seniorOneCollapsed) return;
  seniorOneCollapsed=true;
  const input=$("messageReply");
  input.disabled=true;
  $("messageSend").disabled=true;
  const laughs=[
    "ㅋㅋㅋㅋㅋ",
  ...
  laughs.forEach((line,i)=>later(()=>addMessageBubble(line,"in","선배 1"),380+i*260));
  for(let i=0;i<18;i++){
    later(()=>addMessageBubble("ㅋ".repeat(26+Math.floor(Math.random()*110)),"in",""),1750+i*115);
  }
  later(()=>{
    state.pollution=100;
    updateStatus();
    swarmBurst(120,"ㅋㅋㅋㅋㅋㅋㅋㅋㅋㅋㅋㅋㅋㅋ");
    const overlay=$("messageOverlay");
    overlay?.classList.add("signal-lost");
  },3900);
  later(()=>{
    closeMessageThread(true);
    $("messageOverlay")?.classList.remove("signal-lost");
    hideMessageNotice();
    toast("발신 기록이 소실되었습니다.");
  },5000);
}
function closeMessageThread(forced=false){
  const overlay=$("messageOverlay");
  if(cinemaMessageMode && !forced){
    cinemaMessageMode=false;
    overlay?.classList.remove("open","cinema-message-glitch","cinema-black-message");
    overlay?.setAttribute("aria-hidden","true");
    document.querySelector(".message-compose")?.classList.remove("cinema-compose-hidden","cinema-compose-dead");
    if($("messageReply")) $("messageReply").placeholder="메시지";
    setTimeout(showCinemaMissingEnding,380);
    return;
  }
  overlay?.classList.remove("open");
  overlay?.setAttribute("aria-hidden","true");
  if(!forced && selectedSenior==="2" && clueAnswerDelivered && recoveryQueued && !recoveryShownThisRun){
    recoveryQueued=false;
    recoveryShownThisRun=true;
    swarmBurst(45,"재난관리국을 아직도 믿고 있어?");
    setTimeout(openRecoveryTransmission,450);
  }
}
function sendAgentReply(){
  const input=$("messageReply");
  if(!input || input.disabled || !selectedSenior) return;
  const raw=input.value.trim();
  if(!raw) return;
  addMessageBubble(raw,"out");
  input.value="";
  ...
  if(selectedSenior==="1"){
    if(["알겠어요","알겠습니다","네","ㅇㅇ","응"].includes(normalized)){
      later(triggerSeniorOneCollapse,500);
    }else{
      later(()=>{
        addMessageBubble("알겠다고 해.","in","선배 1");
        input.disabled=false;$("messageSend").disabled=false;input.focus();
      },650);
    }
    return;
  }

  if(["네","예","ㅇㅇ","응","yes","y"].includes(normalized)){
    later(()=>{
      addMessageBubble(RECOVERY_ANSWER,"in","선배 2");
      clueAnswerDelivered=true;
      recoveryQueued=true;
      input.placeholder="문자창을 닫아 통신에 응답하십시오";
      addMessageBubble("기억했다면 이 대화를 닫으십시오.","in","선배 2");
    },800);
  }else{
    later(()=>{
      addMessageBubble("응답을 확인할 수 없습니다. 희생자를 구하고 싶다면 ‘네’라고 답하십시오.","in","선배 2");
      input.disabled=false;$("messageSend").disabled=false;input.focus();
    },700);
  }
}

$("messageNotice")?.addEventListener("click",openMessageThread);
$("messageNotice")?.addEventListener("keydown",e=>{if(e.key==="Enter"||e.key===" ") openMessageThread();});
$("messageClose")?.addEventListener("click",()=>closeMessageThread(false));
$("messageSend")?.addEventListener("click",sendAgentReply);
$("messageReply")?.addEventListener("keydown",e=>{if(e.key==="Enter"){e.preventDefault();sendAgentReply();}});
$("messageOverlay")?.addEventListener("click",e=>{if(e.target===$("messageOverlay")) closeMessageThread(false);});

checkRecoveryTransmission = function(){
  if(state.pollution < 82 || recoveryShownThisRun || selectedSenior==="1") return;
  if(!criticalSince) criticalSince=Date.now();
  const elapsed=Date.now()-criticalSince;
  if(elapsed >= 2400 && !clueNoticeShown){
    clueNoticeShown=true;
    showMessageNotice("알 수 없는 선배 요원 두 명에게서 문자가 도착했습니다.");
  }
};

/* Preserve the message state reset between complete runs. */
const resetStateBeforeMessage = resetState;
resetState = function(){
  resetStateBeforeMessage();
  clueNoticeShown=false;clueThreadOpened=false;selectedSenior=null;clueAnswerDelivered=false;recoveryQueued=false;seniorOneCollapsed=false;
  clearMessageTimers();hideMessageNotice();closeMessageThread(true);
  if($("messageThread")) $("messageThread").innerHTML="";
};

/* ===== CINEMA HIDDEN ROUTE ===== */
const CINEMA_RECITAL = "이것은 참다운 법으로써 여래를 공양하는 길이오니 나라를 다 바치고 처자로 보시하여도 이것이 제일의 보시이옵니다";

function cinemaRouteEligible(){
  return !!(
    state.seenChoices.a1_hand &&
    state.seenChoices.q_observe &&
    state.seenChoices.s_door &&
    state.seenChoices.door_talk &&
    state.questions === 0
  );
}

/* Replace the door-answer scene so the ticket appears only on the exact route. */
SCENES.guestTalk.choices = ()=>{
  const list=[];
  if(cinemaRouteEligible()){
    list.push(makeChoice("주머니에서 무언가 만져진다.","cinemaTicket",{key:"cinema_pocket"}));
  }
  list.push(makeChoice("뒤로 물러난다.","act4",{key:"guest_leave"}));
  return list;
};


function playCinemaTicketTear(nextScene){
  if(document.querySelector(".ticket-tear-overlay")) return;
  const overlay=document.createElement("div");
  overlay.className="ticket-tear-overlay";
  overlay.setAttribute("role","dialog");
  overlay.setAttribute("aria-label","영화표 찢기");
  overlay.innerHTML=`
    <div class="ticket-tear-vignette"></div>
    <div class="ticket-tear-stage">
      <div class="ticket-half ticket-half-left"><img src="cinema-ticket.png" alt="피와 부적이 묻은 낡은 영화표"></div>
      <div class="ticket-half ticket-half-right"><img src="cinema-ticket.png" alt=""></div>
      <div class="ticket-rip-line" aria-hidden="true"></div>
      <p class="ticket-tear-caption">손끝에 힘을 주십시오.</p>
      <button class="ticket-tear-trigger" type="button">화면을 터치하여 찢기</button>
      <div class="ticket-shreds" aria-hidden="true"></div>
    </div>`;
  document.body.appendChild(overlay);
  document.body.classList.add("ticket-cutscene-open");
  requestAnimationFrame(()=>overlay.classList.add("ready"));
  const trigger=overlay.querySelector(".ticket-tear-trigger");
  let torn=false;
  const tear=()=>{
    if(torn) return; torn=true;
    trigger.disabled=true;
    overlay.classList.add("tearing");
    const holder=overlay.querySelector(".ticket-shreds");
    for(let i=0;i<12;i++){
      const bit=document.createElement("i");
      bit.style.setProperty("--x",`${(Math.random()-.5)*440}px`);
      bit.style.setProperty("--y",`${80+Math.random()*260}px`);
      bit.style.setProperty("--r",`${-80+Math.random()*160}deg`);
      bit.style.setProperty("--d",`${Math.random()*.18}s`);
      holder.appendChild(bit);
    }
    setTimeout(()=>overlay.classList.add("blackout"),850);
    setTimeout(()=>{
      document.body.classList.remove("ticket-cutscene-open");
      overlay.remove();
      renderScene(nextScene);
      document.body.classList.add("cinema-projector-enter");
      setTimeout(()=>document.body.classList.remove("cinema-projector-enter"),1800);
    },1500);
  };
  trigger.addEventListener("click",tear,{once:true});
  overlay.addEventListener("click",e=>{if(e.target===overlay) tear();});
}

SCENES.cinemaTicket={
  act:3,code:"UNREGISTERED ITEM",title:"영화표",
  text:()=>`주머니 안에서 낡은 영화표 한 장이 나온다.\n\n발권 일시도, 상영관도 적혀 있지 않다.\n\n검게 번진 좌석 번호 아래에는 네 개의 문장이 인쇄되어 있다.`,
  choices:()=>[
    makeChoice("찢는다.","cinemaArrival",{key:"cinema_tear"}),
    makeChoice("기억하세요. 당신은 희생자를 찾으러 왔습니다.","cinemaRefusal",{key:"cinema_remember"}),
    makeChoice("당신은 재난관리국의 요원입니다.","cinemaRefusal",{key:"cinema_agent"}),
    makeChoice("당신 자신을 잃지 마세요.","cinemaRefusal",{key:"cinema_self"})
  ]
};

SCENES.cinemaRefusal={
  act:3,code:"ITEM REJECTED",title:"접히지 않은 표",
  text:()=>`문장을 읽는 동안 영화표의 글자가 천천히 지워진다.\n\n마지막까지 남은 것은 찢어진 자국처럼 보이는 검은 선 하나뿐이다.\n\n기묘한 이끌림은 사라졌다.`,
  choices:()=>[makeChoice("두 번째 질문으로 넘어간다.","act4",{key:"cinema_refuse_exit"})]
};

SCENES.cinemaArrival={
  act:0,code:"SCREEN 00",title:"상영 전",
  onEnter:()=>{document.body.classList.add("cinema-mode"); renderRules();},
  text:()=>`기묘한 이끌림을 따라 영화표를 찢는 순간 어두컴컴한 공간으로 이동되었다.\n\n주변을 둘러보자 제 앞에 앉아 있는 신원 불명의 인영이 보인다.`,
  choices:()=>[
    makeChoice("여자에게 말을 건다.","cinemaDeath",{key:"cinema_talk"}),
    makeChoice("영화가 시작할 때까지 기다린다.","cinemaRecital",{key:"cinema_wait"})
  ]
};

SCENES.cinemaRecital={
  act:0,code:"SCREEN 01",title:"상영 직전",
  text:()=>`스크린에 자막 한 줄이 떠오른다.\n\n입술을 움직여 정확히 소리 내어 말해야 한다.\n한 글자라도 다르면 영화는 시작되지 않는다.`,
  choices:()=>[]
};

ENDINGS.cinemaDeath={
  code:"HIDDEN END · DEATH",title:"새로운 관객",
  noBadge:true,
  text:()=>`앞좌석의 여자에게 말을 건 순간, 인영의 머리가 천천히 뒤로 꺾인다.\n\n얼굴이 있어야 할 자리에는 아직 시작되지 않은 영화의 화면만이 비치고 있다.\n\n그것이 ${state.keyword}의 이름을 부른다.\n\n당신은 영화관에서 사망했습니다.`,
  summary:()=>"재난관리국은 회수할 사원증을 발견하지 못했습니다."
};

function mountCinemaRecitalInput(){
  if(currentSceneId!=="cinemaRecital") return;
  const choices=$("choiceList");
  if(!choices || $("cinemaRecitalInput")) return;
  choices.innerHTML=`
    <div class="cinema-recital-box">
      <label for="cinemaRecitalInput">스크린의 문장을 입력하십시오.</label>
      <textarea id="cinemaRecitalInput" rows="5" spellcheck="false" autocomplete="off" placeholder="문장을 정확히 입력하십시오."></textarea>
      <p id="cinemaRecitalFeedback" aria-live="polite"></p>
      <button id="cinemaRecitalSubmit" class="primary-button" type="button">소리 내어 말한다</button>
    </div>`;
  const submit=()=>{
    const input=$("cinemaRecitalInput");
    const normalized=input.value.replace(/[\s.!?,，。‘’“”"']/g,"");
    const answer=CINEMA_RECITAL.replace(/[\s.!?,，。‘’“”"']/g,"");
    if(normalized!==answer){
      $("cinemaRecitalFeedback").textContent="문장이 일치하지 않습니다. 스크린이 다시 처음부터 깜빡입니다.";
      addPollution(8);
      input.classList.add("wrong");
      setTimeout(()=>input.classList.remove("wrong"),500);
      return;
    }
    input.disabled=true;$("cinemaRecitalSubmit").disabled=true;
    showCinemaGlitchMessage();
  };
  $("cinemaRecitalSubmit").addEventListener("click",submit);
  $("cinemaRecitalInput").addEventListener("keydown",e=>{if(e.key==="Enter"&&(e.ctrlKey||e.metaKey)){e.preventDefault();submit();}});
}

let cinemaMessageMode=false;
function showCinemaGlitchMessage(){
  cinemaMessageMode=true;
  clearMessageTimers();
  hideMessageNotice();
  const overlay=$("messageOverlay");
  const thread=$("messageThread");
  overlay.classList.add("cinema-black-message","cinema-message-glitch","open");
  overlay.setAttribute("aria-hidden","false");
  setMessageHeader("선배 요원","선");
  thread.innerHTML='<div class="message-time">지금 · 연결 상태를 확인할 수 없음</div>';
  $("messageReply").disabled=true;
  $("messageSend").disabled=true;
  const compose=document.querySelector(".message-compose");
  compose?.classList.remove("cinema-compose-hidden");
  compose?.classList.add("cinema-compose-dead");
  if($("messageReply")) $("messageReply").placeholder="답장할 수 없습니다";
  later(()=>addMessageBubble("미안해","in","선배 요원"),450);
  later(()=>addMessageBubble("나를 용서해 줘","in","선배 요원"),1450);
  later(()=>{
    const note=document.createElement("div");
    note.className="cinema-close-note";
    note.textContent="대화를 닫으십시오.";
    thread.appendChild(note);
  },2600);
}

function showCinemaMissingEnding(){
  state.endingId="cinemaMissing";
  $("endingCode").textContent="HIDDEN END · MISSING";
  $("endingTitle").textContent="상영 종료 후 실종";
  $("endingText").className="ending-text cinema-final-message";
  $("endingText").innerHTML=`<div class="cinema-final-phone"><div class="cinema-final-head">메시지 <span>지금</span></div><div class="cinema-final-bubble">실종된 당신을 찾기 위해 재난관리국 요원들이 곧 파견될 예정입니다.</div></div>`;
  $("endingKeyword").textContent=state.keyword;
  $("endingSummary").textContent="회수된 신분 기록이 없습니다.";
  $("agentCardTitle")?.closest(".agent-card-maker")?.classList.add("hidden-ending-badge");
  $("saveResultButton").style.display="none";
  showScreen("endingScreen");
  document.body.classList.add("cinema-mode","cinema-ending");
}

/* Keep track of the rendered scene and mount the exact-phrase input. */
currentSceneId="";
const renderSceneBeforeCinema=renderScene;
renderScene=function(id){
  currentSceneId=id;
  renderSceneBeforeCinema(id);
  if(id==="cinemaRecital") setTimeout(mountCinemaRecitalInput,0);
};

/* Hide employee-card issuance for every cinema death/missing ending. */
const showEndingBeforeCinema=showEnding;
showEnding=function(id){
  showEndingBeforeCinema(id);
  const noBadge=!!ENDINGS[id]?.noBadge;
  $("agentCardTitle")?.closest(".agent-card-maker")?.classList.toggle("hidden-ending-badge",noBadge);
  $("saveResultButton").style.display=noBadge?"none":"";
  if(id==="cinemaDeath") document.body.classList.add("cinema-mode","cinema-ending");
};

const resetStateBeforeCinema=resetState;
resetState=function(){
  resetStateBeforeCinema();
  document.body.classList.remove("cinema-mode","cinema-ending");
  $("agentCardTitle")?.closest(".agent-card-maker")?.classList.remove("hidden-ending-badge");
  if($("saveResultButton")) $("saveResultButton").style.display="";
  $("cinemaGlitchMessage")?.remove();
  cinemaMessageMode=false;
  $("messageOverlay")?.classList.remove("cinema-message-glitch","cinema-black-message");
  document.querySelector(".message-compose")?.classList.remove("cinema-compose-hidden","cinema-compose-dead");
  if($("messageReply")) $("messageReply").placeholder="메시지";
  renderRules();
  currentSceneId="";
